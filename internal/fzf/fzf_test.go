...
func TestSessionLegend(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantContains:   []string{"ctrl-t: hide inactive", "enter: switch", "ctrl-d: delete", "ctrl-r: rename", "ctrl-s: tools"},
			wantNotContain: []string{"ctrl-t: show all"},
		},
		{
			name:           "filtered view shows show all hint",
			showActiveOnly: true,
			wantContains:   []string{"ctrl-t: show all", "enter: switch", "ctrl-d: delete", "ctrl-r: rename", "ctrl-s: tools"},
